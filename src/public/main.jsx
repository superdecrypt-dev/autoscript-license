      ...
      <section className="overflow-hidden rounded-[2rem] border border-[var(--line)] bg-[var(--panel)] shadow-[var(--shadow)]">
        <div className="grid gap-8 px-6 py-8 md:px-10 md:py-12 lg:grid-cols-[1.2fr,0.8fr]">
          ...
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-1">
            <InfoPill label="Renew Window" value={`${renewOpenBeforeDays} hari`} tone="amber" />
            <InfoPill label="Support" value="autoscript@atomicmail.io" tone="slate" />
            <InfoPill label="Worker" value={statusBadge.message} tone={statusBadge.tone} />
          </div>
        </div>
      </section>
...
function ResultPanel({ result }) {
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-[var(--line)] bg-white/70 p-4">
